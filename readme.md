...
Разбивает длинное число на группы из трех символов. Например
...
Необязательый аргумент, гиперссылка. Если не передан, будет использоваться window.location.href.
...
```javascript
  let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=second#hash';
  s = HttpQueryString.setVariable(s, 'ecver', 'newvalue');
  // s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=newvalue&autoplay=1&c[]=first&c[]=second#hash'
```

Если в качестве имени переменной передать имя массива, замена будет произведена для всех вхождений.

 ```javascript
  let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=second#hash';
  s = HttpQueryString.setVariable(s, 'c[]', 'newvalue');
  // s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=newvalue&c[]=newvalue#hash';
```

Я долго думал, стоит ли так делать, но DeepSeek считает, что [это логично для большинства разработчиков](https://chat.deepseek.com/share/noplxl022teznue9c3)

В случае, если надо изменить значение только одного элемента массива следует воспользоваться аргументами `checkByValue` и `unsetValue`:

```javascript
  let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=second#hash';
  s = HttpQueryString.setVariable(s, 'c[]', 'newvalue', true, 'first');
  // s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=newvalue&c[]=second#hash';
```

```javascript
  let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=second#hash';
  s = HttpQueryString.setVariable(s, 'c[]', 'newvalue', true, 'second');
  // s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=newvalue#hash';
```
...
Это значение будет установлено как значение переменной `varName` в строке http запроса (или в ссылке, что по сути одно и то же) `link`. Специальное значение `'CMD_UNSET'` используется если мы хотим удалить переменную из строки запроса.
...
```javascript
	let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=second#hash';
	s = HttpQueryString.setVariable(s, 'c[]', 'CMD_UNSET', true, 'second');
  //s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first#hash'
```

При этом можно удалить все вхождения `c` в строку:
	
```javascript
	let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[]=first&c[]=second#hash';
	s = HttpQueryString.setVariable(s, 'c[]', 'CMD_UNSET');
  // s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1#hash'
```

В случае массива с ключами логично использовать `имя_переменной[ключ]` для установки.

```javascript
  let s = 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=2&autoplay=1&c[145]=first&c[20111]=second#hash';
	s = HttpQueryString.setVariable(s, 'c[20111]', 'ollama');
	// s == 'https://www.youtube.com/embed/xqagdM4BmNs?ecver=newvalue&autoplay=1&c[145]=first&c[20111]=ollama#hash'
```
...
Change the values ​​of variables in the query string.
...
If you pass an array name as a variable name, the replacement will be performed for all occurrences.
...
I thought for a long time whether it was worth doing this, but DeepSeek thinks that [it is logical for most developers](https://chat.deepseek.com/share/noplxl022teznue9c3)


If you need to change the value of only one element of the array, you should use the `checkByValue` and `unsetValue` arguments:
